...
export async function POST(request: Request) {
  ...
  const { date } = res as JournalEntry;
  ...
  // Remove the entry with the given date from the existing data array
  existingData.journal = existingData.journal.filter((entry) => entry.date !== date);

  // Write the updated data back to journal.json
  fs.writeFileSync(dataFilePath, JSON.stringify(existingData, null, 2));

  return new Response('Data removed from journal.json successfully.');
}
